...
export default function TradingSetting() {
  ...
  const backToDefault = async () => {
    try {
      await defaultSetting(DEFAULT_USER_SETTINGS);
      setUser((prev) => ({
        ...prev,
        ...DEFAULT_USER_SETTINGS,
      }));
      setFormTrade(DEFAULT_USER_SETTINGS);
      toast.success("Changed Successfully");
    } catch (error: any) {
      const serverMessage = error?.response?.data?.message;
      const generalMessage = error?.message;
    ...
    <div className="flex flex-col gap-px">
      {[
        {
          ...
          control: (
            <select
              ...
              value={formTrade.account_currency}
              className="w-64 h-9 bg-zinc-950 border border-zinc-800 hover:border-zinc-700 focus:border-zinc-600 rounded-lg px-3 text-sm text-white focus:outline-none transition-all appearance-none"
            >
        ...
        },
        {
          ...
          control: (
            <select
              ...
              value={formTrade.trading_experience}
              className="w-64 h-9 bg-zinc-950 border border-zinc-800 hover:border-zinc-700 focus:border-zinc-600 rounded-lg px-3 text-sm text-white focus:outline-none transition-all appearance-none"
            >
      ...
      <div className="flex items-center justify-between pt-6">
        <button
          ...
          className="text-sm text-zinc-600 hover:text-zinc-400 transition-all"
        >
          Reset to defaults
        </button>
