      ...
      <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto flex-1 p-6 pb-24">
        ...
        <div className="w-full p-5 rounded-lg border border-zinc-900 bg-zinc-950 flex flex-col gap-3">
          ...
          <div className="text-sm text-zinc-500 min-h-[60px] leading-relaxed whitespace-pre-wrap">
            {trade.notes ? <span className="text-zinc-300">{trade.notes}
            </span> : "No textual metadata or discretionary annotations appended to this sequence block."}
          </div>
        </div>
      </div>
      ...
      {screenshots && screenshots.length > 0 && (
        <div className="w-full max-w-7xl mx-auto px-6 pb-24 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            description="Are you sure to Delete this screensShot permantly" 
            onClose={() => setDeleteImageModal(false)} 
            onDelete={() => handleDelete(deleteImage.screenshot,deleteImage.index)}
            loading={deleteScreenshotLoading}/>
             }
          </div>
        </div>
      )}
